...
const fetchQRCode = async () => {
  try {
    const accessToken = localStorage.getItem('accessToken');
    const response = await fetch(
      `http://localhost:4004/employes/${employeId}/qrcode`,
      { headers: { 'Authorization': `Bearer ${accessToken}` } }
    );
    const data = await response.json();
    if (data.success) setQrCodeImage(data.data.qrCodeImage);
  } catch (error) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-green-50 rounded-xl p-6 shadow-sm border border-green-200">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-medium text-green-800">Assiduité</p>
                <TrendingUp className="w-8 h-8 text-green-600" />
              </div>
              <p className="text-3xl font-bold text-green-900">{stats.tauxPresence}%</p>
