import { Edit, Eye, Plus, Search, RefreshCw, User, Trash2 } from 'lucide-react';
import React, { useEffect, useState } from 'react';
import { ApiEntreprise } from '../../api/apiEntreprise';
import { useNavigate } from 'react-router-dom';
import ModalEntreprise from '../Modals/modalEntreprise';
import ModalEntrepriseDetails from '../Modals/modalDetail';
import Header from '../../layout/header';
import Sidebar from '../../layout/sidebar';
import Pagination from '../Pagination'; // ✅ nouveau composant
...
export default function EntrepriseList({ notifications, activeLink, setActiveLink }) {
  ...
  const [selectedPeriod, _setSelectedPeriod] = useState('6mois');
  ...
  const fetchEntreprises = async () => {
    try {
      ...
      const sanitizedEntreprises = entreprisesArray.map((e) => ({
        ...e,
        employes: e.employes || [],
        masseSalariale: e.masseSalariale || 0,
        evolution: e.evolution || 0,
        statut: e.statut || 'inactif',
      }));

      setEntreprises(sanitizedEntreprises);
    } catch (err) {
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {!loading && paginatedEntreprises.length > 0 &&
                    paginatedEntreprises.map((item) => (
